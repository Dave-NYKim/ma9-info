...
import { useCreatePitcher, useUpdatePitcher, useDeletePitcher, usePitcherNameSearch } from '@entities/pitcher'
import { codeLabel, codeValues, teamsByLeague, useCodes } from '@entities/code'
import { usePitcherPotentials, useSpecialPitches, useCreatePotential } from '@entities/potential'
...
export function PitcherForm({ initial, onDone, onCancel }: { initial?: PitcherWithPitches; onDone: () => void; onCancel: () => void }) {
  ...
  const del = useDeletePitcher()
  const createPot = useCreatePotential('pitcher')
  ...
  const removePlayer = async () => {
    if (!initial) return
    const r = askDeleteCode(`${initial.name} · ${initial.grade}등급 투수`)
    if (r === 'cancel') return
